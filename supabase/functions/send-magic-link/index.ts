...
// Generates a magic login link for an employee and emails it via Resend.
// Callable by company admins (not just superadmin).
...
serve(async (req: Request) => {
  ...
  try {
    ...
    // ── Check caller is admin/manager of their company (unless superadmin) ──────
    ...
    if (!isSuperAdmin) {
      ...
      if (roleErr || !callerRole) throw new Error('Unauthorized — no role found')

      const allowedRoles = ['admin', 'manager', 'superadmin']
      if (!allowedRoles.includes(callerRole.role)) {
        throw new Error('Only admin or manager can generate login links')
      }
    }
          ...
          html: `
            <div style="font-family:sans-serif;max-width:480px;margin:0 auto;padding:32px 24px;background:#0f172a;color:#f1f5f9;border-radius:12px;">
              ...
              <p style="color:#94a3b8;font-size:14px;margin:0 0 28px;">
                Your manager has sent you a login link for Nhance. Click the button below to sign in — no password needed. This link is valid for <strong style="color:#f1f5f9;">1 hour</strong> and can only be used once.
              </p>
