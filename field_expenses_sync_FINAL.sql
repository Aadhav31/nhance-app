-- ============================================================
-- Field Expenses → Accounts Sync  — FINAL (all ENUMs fixed)
-- Open a FRESH tab in Supabase SQL Editor, paste all, Run
-- ============================================================


-- Step 1: Convert ENUM columns → TEXT
--   Both category AND payment_mode are custom ENUMs.
--   This MUST run before any INSERT that puts text into them.
-- ============================================================
...
--   payment_mode already exists (as text now), so it is skipped.
-- ============================================================
ALTER TABLE expenses
...
-- Step 5: INSERT trigger function
--   Fires on every field_expenses insert (web app, APK, anything).
--   All columns in expenses are now text — no type conflicts.
-- ============================================================
...
BEGIN
  ...
  INSERT INTO expenses (
    ...
    0,
    COALESCE(NEW.amount, 0),   -- total_amount = amount + tax (tax=0 for field expenses)
    v_pay_mode,
    NEW.transaction_ref,
    NEW.equipment_id,
    NEW.created_by,
...
SELECT
  ...
  COALESCE(fe.category::text, 'misc'),
  CASE
    ...
    ELSE REPLACE(INITCAP(COALESCE(fe.category::text, 'Expense')), '_', ' ')
  END,
  fe.payee_name,
  COALESCE(fe.amount, 0),
  0,
  COALESCE(fe.amount, 0),   -- total_amount
